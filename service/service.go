...
func (s *HttpService) GetIllegalEvidenceByHeight(param util.Params) (interface{}, error) {
	height, ok := param.Uint("height")
	if !ok {
		return nil, util.NewError(int(InvalidParams), "height parameter should be a positive integer")
	}
...
func (s *HttpService) CheckIllegalEvidence(param util.Params) (interface{}, error) {
	evidence, ok := param["evidence"]
	if !ok {
		return nil, util.NewError(int(InvalidParams), "no evidence")
	}
